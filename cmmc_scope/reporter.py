...
class _CMMCReportPDF(FPDF):
    ...
    def draw_finding_section(self, finding: Finding, index: int) -> None:
        ...
        self.cell(_PDF_CONTENT_WIDTH_MM - 25, 6, _safe(id_label), new_x=XPos.RIGHT)
        ...
        meta = (
            f"NIST 800-171: Section {finding.nist_control_id}  |  "
            f"Family: {finding.control_family}  |  "
            f"Scope: {finding.resource_scope}  |  "
            f"Evaluated: {finding.evaluated_at}"
        )
        self.cell(0, 5, _safe(meta), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.set_text_color(*_COLOR_BLACK)
        self.ln(1)
        ...
        self.set_font("Helvetica", "", 9)
        self.set_x(_PDF_MARGIN_MM)
        self.multi_cell(_PDF_CONTENT_WIDTH_MM, 5, _safe(finding.summary))
        self.ln(2)
        ...
        if finding.evidence:
            ...
            for line in finding.evidence:
                ...
                self.multi_cell(
                    _PDF_CONTENT_WIDTH_MM - 2, 4.5, _safe(line), fill=True
                )
            self.ln(1)
        ...
        if finding.remediation_items:
            ...
            for i, item in enumerate(finding.remediation_items, start=1):
                self.set_x(_PDF_MARGIN_MM + 2)
                self.multi_cell(_PDF_CONTENT_WIDTH_MM - 2, 5, _safe(f"{i}. {item}"))
